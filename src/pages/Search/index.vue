        ...
        <div class="details clearfix">
          ...
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <!-- 综合、价格排序；升序、降序排序 -->
                <li :class="{ active: isActive }" @click="sort('1')">
                ...
                <li :class="{ active: !isActive }" @click="sort('2')">
                  <a href="javascript:void(0)"
                    >价格
                    <span v-show="!isActive">
                      <i v-show="upArrow">↑</i>
...
export default {
  ...
  methods: {
    ...
    clearId() {
      // 也可以将空字符串写成undefined，这样axios会自动帮我们把undefined给省去不发送请求。可以轻微的降低服务器的负担
      // 但是这里为了以后复习清晰明确，还是使用语义化更好的空字符串
    ...
    removeKeyword() {
      ...
      this.$bus.$emit("clearInputValue");
    },
    // 给SearchSelector的自定义事件
    ...
    sort(flag) {
      ...
      if (this.searchParams.order.split(":")[0] == flag) {
        ...
        this.searchParams.order = `${flag}:${
          this.searchParams.order.split(":")[1] == "desc" ? "desc" : "asc"
        }`; // 2:desc
      }
      this.getData();
    },
